            ...
            <div class="card">
              ...
              <div class="card-body">
                <form class="form-group" action="{{ route('policies.store') }}" method="post">
                  @csrf
                  <div class="py-3">
                    <input class="form-control" type="text" name="policy_heading" placeholder="Enter heading?" value="{{ old('policy_question') }}">
                    @error('policy_question')
                      <small class="text-danger">{{ $message }}</small>
                    @enderror
                  </div>
                  <div class="py-3">
                    <input class="form-control" type="text" name="policy_details" placeholder="Answer" value="{{ old('policy_answer') }}">
                    @error ('policy_answer')
                      <small class="text-danger">{{ $message }}</small>
                    @enderror
                  </div>
